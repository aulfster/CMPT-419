...
class Decoder:
    ...
    def read_weights(self, weight_file):
        ...
        file.seek(0)
        for line in file:
            # will split each line with format /// TAG/SUFF:word:tag weight /// according to default delimiter, space. 
            # So, there would be 2 tokens- token[0] = TAG/SUFF:word:tag and token[1] = weight
            token = line.split()
            feature = token[0]
            #print token[1]
            weight  = float(token[1])
            self.f.add(feature)
            # v is a dictionary, so store each weight value using its corresponding key, feature string.
            self.v[feature] = weight

            # Add the tags from any trigram features to the list of tags
            # feature string is in the format, TAG/SUFF:word:tag, so using split on delimiter ':', split the word and the tag 
            token = feature.split(':')
            if token[0] == TRIGRAM:
                # The method update() adds set, token[1:], word-tag pairs in to set, self.tags. This function does not return anything.
...
#         for feature in self.f:
    ...
    def tag_words(self, data_file):
        ...
        for line in file: 
            ...
            #elif not line:
                #break;
            else: #if line.replace( '\r' , '' ) == "\n" or not line :  End of sentence reached
...
if __name__ == "__main__":
    main("NER_weightlist.txt","eng_processed_unlabeled.testa")
